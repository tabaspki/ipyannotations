...
from .canvases.image_utils import load_img
...
class ZoomController(widgets.VBox):
    ...
    def __init__(self, mask: Union[str, pathlib.Path, Image.Image], width: int = 200, height: int = 200):
        self.canvas = ZoomCanvas(width=width, height=height)
        self.mask_canvas = load_img(mask)
        self.mask_canvas.width=200
        self.mask_canvas.height=200
        self.mask_canvas.layout.border = "1px solid black"
        ...
        super().__init__()
        self.children = [zoom_controls, self.canvas, self.mask_canvas]
    ...
    def load_image(self, image: Union[str, pathlib.Path, Image.Image], mask: Union[str, pathlib.Path, Image.Image]):
        self.canvas.load_image(image)
        self.mask_canvas.value = (load_img(mask)).value
